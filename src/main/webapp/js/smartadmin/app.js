...
$(document).ready(function() {
	...
	// EVENTS
	$('#events-menu').click(function(e) {
		
		$('#events-box').toggle("slow");
	...
	$('#logout a').click(function(e) {
		...
		$.logoutTitle = $this.data('logout-title');
		$.logoutMSG = $this.data('logout-msg');

		// ask verification
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> " + $.logoutTitle,
			content : $.logoutMSG,
			buttons : '[No][Si]'
...
function nav_page_height() {
	...
	var height = $('#content').height() + 150;

	$.root_.css('min-height', height + 34 + 'px');
	
	$.left_panel.css('min-height', height + 'px');
	$.main_.css('min-height', height + 'px');
	...
	if (height > windowHeight) {
		...
		$.left_panel.css('min-height', windowHeight - 49 + 'px');
		$.main_.css('min-height', windowHeight - 49 + 'px');
	}
...
* Description: Executes and fetches all ajax requests also updates navigation elements to active
*/
...
function loadURL(url, container) {
	...
	$.ajax({
		...
		error : function(xhr, ajaxOptions, thrownError) {
			loadError('/error/' + xhr.status, container);	
		},
		async : false
	});
...
function loadError(url, container) {
	...
	$.ajax({
		...
		error : function(xhr, ajaxOptions, thrownError) {
			container.html('<h4 style="margin-top:10px; display:block; text-align:left"><i class="fa fa-warning txt-color-orangeDark"></i> Unknown Error!</h4>');
		},
		async : false
	});
}

function navigateURL(url, container) {
...
function drawBreadCrumb() {
	...
	nav_elems.each(function() {
		...
		if (!--count) {
			document.title = '2me - ' + $.bread_crumb.find("li:last-child").text();
		}
	});
